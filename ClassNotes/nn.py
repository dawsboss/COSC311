...
class NN:
    def __init__(self, d=2, nl=1, ls=2, r=.1, b=False):
    ...
    def reset(self):
        ...
        self.w_hidden = np.random.standard_normal((num_layers-1, layer_size, layer_size))
        self.w_out = np.random.standard_normal((len(parse_crimes),layer_size))
...
"""
    def backprop(self, outputs, pred, data, label):
        ...
        dExo = dEyo * sigmoid_deriv(outputs[-1] @ self.outw.T) # vector 1xM @ (1xL @ LxM)
        ...
        dEyh[-1] = dExo @ self.outw  #  1xM @ MxL

        for i in range(self.num_layers-2,-1,-1):
            # i-1 to get the inputs to layer i
            x = outputs[i-1] @ self.hiddenw[i-1] # 1-by-h times h-by-h
            dExh[i] = dEyh[i] * sigmoid_deriv(x) # 1-by-h
            dEwh[i] = outputs[i-1] * dExh[i]
            if i > 0:
                # prep the next layer
                dEyh[i-1] = self.hiddenw[i] @ dExh[i].T # h-by-h times h-by-1
        ...
        self.hiddenw = self.hiddenw - self.rate * dEwh
        self.inw = self.inw - self.rate * dEwi
        self.outw = self.outw - self.rate * dEwo
